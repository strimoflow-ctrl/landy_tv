...
export default function VideoPlayer({ video, onClose, onSelectVideo }) {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      const rel = await fetchRelatedVideos(video.url);
      if (isMounted) {
  ...
  return (
    <div className="player-overlay">
      ...
      {/* Bottom Scrollable Section (Smooth momentum scroll) */}
      <div className="player-suggested-scrollable" ref={scrollContainerRef}>
        <div className="suggested-header-bar">
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Sparkles size={14} color="var(--accent)" />
            <span className="suggested-heading">Up Next & Recommendations</span>
          </div>
          <span className="suggested-count">{loadingRelated ? 'Fetching...' : `${relatedVideos.length} videos`}</span>
        </div>

        {/* Shimmer skeleton while loading suggestions */}
        {loadingRelated && (
